...
class WPCustomerReviews {
 ...
 $cou = $row[0]->total;
	//var_dump( $cou );
        $arr_Reviews = $this->get_reviews($postid, $thispage, ceil($cou/2), 1);
	    ...
	    $reviews_content .= '<div class="comments"><div class="comments-left">';
            foreach ($reviews as $review) {
                ...
                if ($hide_response == 0)
                {
                    if (strlen($review->review_response) > 0) {
                        $review_response = '<p class="response"><strong>Ответ:</strong> ' . nl2br($review->review_response) . '</p>';
                    }
                }
                    ...
                    foreach ($this->options['field_custom'] as $i => $val) {  
                        if ( isset($custom_fields_unserialized[$val]) ) {
                            ...
                            if ($show == 1 && $custom_fields_unserialized[$val] != '') {
                                $custom_shown .= "<div class='wpcr_fl'>" . $custom_fields_unserialized[$val] . '&nbsp;&bull;&nbsp;</div>';
                            }
                        }
                    }
                ...
                $name_block = '' .
                    '<div class="comment-author floated">' .
		    '<div class="avatar"><img src="/wp-content/uploads/reviews/' . $review->image . '" alt=""></div>' .
                    '<div class="name">' . $review->reviewer_name . '</div>' .                    
                    '<div class="post">' .$custom_shown . '</div>' .
                    '</div>';
                ...
                if ($this->options['hreview_type'] == 'product') {
                    ...
                    $reviews_content .= '
                        <div class="comment-wrap" id="hreview-' . $review->id . '">
                            <' . $title_tag . ' class="summary ' . $hidesummary . '">' . $review->review_title . '</' . $title_tag . '>
                            <div class="wpcr_fl wpcr_sc">
                                <abbr class="rating" title="' . $review->review_rating . '"></abbr>
                                <div class="wpcr_rating">
                            ...
                            <blockquote class="description"><div class="text">
						<span class="text-arrow"></span><p>' . $review->review_text . '</p>
						</div></blockquote>
                            ' . $review_response . '
			    ' . $name_block . '
	    ...
	    $thispage = ceil($cou/2)+1;
	    $perpage = floor($cou/2);        
	    $arr_Reviews = $this->get_reviews($postid, $thispage, $perpage, 1); 
	    $reviews = $arr_Reviews[0];
       
	    $reviews_content .= '<div class="comments-right">';
	    foreach ($reviews as $review) {
    ...
    function show_reviews_form() {
        ...
        if ($this->options['ask_fields']['fname'] == 1) {
            ...
            $fields .= '<input class="text-input" type="text" placeholder="Ваше имя" id="' . $rand_prefixes[0] . '-fname" name="' . $rand_prefixes[0] . '-fname" value="' . $this->p->fname . '" />';
        }
        ...
        foreach ($this->options['ask_custom'] as $i => $val) {
            if ( isset($this->options['ask_custom'][$i]) ) {
                if ($val == 1) {
                    ...
                    $fields .= '<input class="text-input" type="text" placeholder="Социальный статус" id="custom_' . $i . '" name="custom_' . $i . '" maxlength="150" value="' . $this->p->$custom_i . '" />';
                }
            } 
        }
        ...
        if ($this->options['goto_show_button'] == 1) {
            ...
            $button_html .= '<p><button class="leave-order leave-comment"><a id="wpcr_button_1" href="javascript:void(0);">Оставить отзыв</a></button></p><hr />';
            $out .= $button_html;
        }
        ...
        $out3 = '<div class="pop-up pop-up-comment">
		<div class="close"></div>
		<div class="order">
			<h2 style="color: #fff !important;">Оставить отзыв</h2>
			<h4 style="color: #fff !important;font-weight: 100 !important;">Ваш отзыв будет размещен после проверки <br> администратора сайта:</h4>
			<div class="order-block">
				<form class="wpcrcform" id="wpcr_commentform" method="post" enctype="multipart/form-data" action="javascript:void(0);">
				<input type="hidden" id="frating" name="frating" />
				   
                <div class="wpcr_rating">' . $this->output_rating(0, true) . '</div>
           
					' . $fields . '
			    <div class="comment" contenteditable="true" placeHolder="Ваш отзыв"></div>
			    <textarea id="' . $rand_prefixes[5] . '-ftext" name="' . $rand_prefixes[5] . '-ftext" rows="8" cols="50">' . $this->p->ftext . '</textarea>

                                    <label for="fconfirm2"><input type="checkbox" name="' . $rand_prefixes[7] . '-fconfirm2" id="fconfirm2" value="1" /> Установите этот флажок, чтобы подтвердить, что вы человек.</label>
                                    <div class="wpcr_clear"></div>
                                    <input type="checkbox" name="' . $rand_prefixes[8] . '-fconfirm3" id="fconfirm3" value="1" />

					<div class="file-upload">
						<span class="leave-photo">Прикрепите фотографию:</span>
						<input type="hidden" name="MAX_FILE_SIZE" value="2097152" />
						<input type="file" accept="images/png, image/jpg" name="userfile">
						<span class="not">Файл с расширением .jpg, .png до 2 мб.</span>
					</div>	
...
$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/wp-content/uploads/reviews/';

$uploadfile = $uploaddir . basename($_FILES['userfile']['name']);

if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
    echo "Ваш файл был загружен и успешно отправлен. Спасибо за интерес к нашей компании.\n";
} 

$filename = end(explode('/',$uploadfile)); //Имя файла для прикрепления
	
        $query = $wpdb->prepare("INSERT INTO `$this->dbtable` 
                (`date_time`, `reviewer_name`, `reviewer_email`, `reviewer_ip`, `review_title`, `review_text`, `status`, `review_rating`, `reviewer_url`, `custom_fields`, image, `page_id`) 
                VALUES (%s, %s, %s, %s, %s, %s, %d, %d, %s, %s, %s, %d)", $date_time, $this->p->fname, $this->p->femail, $ip, $this->p->ftitle, $this->p->ftext, 0, 5, $this->p->fwebsite, $custom_insert, $filename, $pageID);

        $wpdb->query($query);

        $admin_link = get_admin_url().'admin.php?page=wpcr_view_reviews';
        $admin_link = "Ссылка на страницу одобрения администратором: $admin_link";

        @wp_mail(get_bloginfo('admin_email'), "WP Customer Reviews: Новый отзыв был размещен в " . date('m/d/Y h:i'), "Новый отзыв был размещен для " . $this->options['business_name'] . " через WP Customer Reviews. \n\nВам нужно будет войти в админку и утвердить этот отзыв, прежде чем появиться на вашем сайте.\n\n{$admin_link}");

        /* returns false for no error */
        return array(false, '<div>Спасибо за ваши комментарии. Все материалы модерируются и в случае одобрения, ваш появится в ближайшее время.</div>');
    }
